///htme_syncVar(cmd_list,group,datatype,newval,varname,prevSyncMap,[addName]);
...
**      Adds {newval} to the {cmd_list} with the {datatype}, if it is 
...
**      ELSE:
**      Adds {newval} to the {cmd_list} with the {datatype} and returns true
**
...
**            If true, before adding the variable itself to the cmd_list, the
...
if (self.syncForce || group[? "type"] != mp_type.SMART) {
   ...
   if (addName) {
       ...
       ds_list_add(cmd_list,buffer_bool, true);
   }
   ds_list_add(cmd_list,datatype, newval);
   ds_map_replace(prevSyncMap,varname,newval);
   return 1;
} else {
  //This map contains the variables as they were last sync.
  oldval = ds_map_find_value(prevSyncMap,varname);
  ...
  */
  if (is_undefined(oldval) || (oldval != newval && toleranceCheck)) {
     //Add
     if (addName) {
        ds_list_add(cmd_list,buffer_string, varname);
     } else {
...
if (!addName) {
   ds_list_add(cmd_list,buffer_bool, false);
}
return 0;